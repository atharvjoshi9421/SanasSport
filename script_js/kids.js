  /* =========================================
        ...
        // 1. Kids Hero Animation - Playful bounce entrance
        const heroTimeline = gsap.timeline();
        heroTimeline.from(".gsap-kid-title", { scale: 0.8, y: 50, opacity: 0, duration: 1.2, ease: "back.out(1.7)" })
                    .from(".gsap-kid-desc", { y: 30, opacity: 0, duration: 1, ease: "power3.out" }, "-=0.8")
                    .from(".gsap-kid-filters .k-filter", { y: 20, opacity: 0, duration: 0.6, stagger: 0.1, ease: "back.out(1.5)" }, "-=0.5");

        // 2. Bento Grid Cards - Bouncy stagger reveal
        ScrollTrigger.batch(".gsap-k-card", {
            onEnter: batch => gsap.to(batch, { 
                opacity: 1, 
                scale: 1, 
                stagger: { each: 0.15, grid: "auto", from: "start" }, 
                duration: 0.8, 
                ease: "back.out(1.4)" 
            }),
            start: "top 85%",
